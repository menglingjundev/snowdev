...
namespace Lixil.Snow.BizLogic
{
	...
	/// 採番の種類マスタのデータアクセス
	/// </summary>
	public class B051201_BizLogic
	{

		//SQL文実行用
		private B051201_DataAccess da;

		public B051201_BizLogic()
		{
			da = new B051201_DataAccess();
		}
		...
		public List<BUMON_KOUJYOU> SelBUMON(string bumon, string bumonCd)
		{
			...
			result = da.SelBUMON(bumon, bumonCd);
			da.Close();
			return result;
		}
		...
		public int InsPartUpdateRireki(string kinouId, string userName, string userId, object hinban, object hinmei, object bumonCd, object kaihatuPerson, object series)
		{
			// イベントログ追加
			Inspector.Add(MethodBase.GetCurrentMethod().DeclaringType.FullName + "." + MethodBase.GetCurrentMethod().Name);
			var result = da.InsPartUpdateRireki(kinouId, userName, userId, hinban, hinmei, bumonCd, kaihatuPerson, series);
			da.Close();
			return result;
		}
		...
		public int InsPartUpdateRireki(string kinouId, string userName, string userId, object tanaNo, object hinban, object hinmei, object bumonCd, object supplier, object kaihatuPerson, object deliveryPerson, object updateProperty, object maeData, object atoData, object series)
		{
			// イベントログ追加
			Inspector.Add(MethodBase.GetCurrentMethod().DeclaringType.FullName + "." + MethodBase.GetCurrentMethod().Name);
			var result = da.InsPartUpdateRireki(kinouId, userName, userId, tanaNo, hinban, hinmei, bumonCd, supplier, kaihatuPerson, deliveryPerson, updateProperty, maeData, atoData, series);
			da.Close();
			return result;
		}
		...
		/// <returns>影響行数</returns>
		/// <remarks>20240109 劉 新規作成</remarks>
		public int InsPartUpdateRireki(string kinouId, string userName, string userId, string hinban, string hinmei, string kaihatuPerson, string updateProperty, string atoData, string series)
		{
			// イベントログ追加
			Inspector.Add(MethodBase.GetCurrentMethod().DeclaringType.FullName + "." + MethodBase.GetCurrentMethod().Name);
			var result = da.InsPartUpdateRireki(kinouId, userName, userId, hinban, hinmei, kaihatuPerson, updateProperty, atoData, series);
			da.Close();
			return result;
		}
		...
		/// <returns>影響行数</returns>
		/// <remarks>20240109 劉 新規作成</remarks>
		public int InsPartUpdateRireki(string kinouId, string userName, string userId, string tanaNo, string hinban, string hinmei, string bumonCd, string kaihatuPerson, string updateProperty, string atoData, string series)
		{
			// イベントログ追加
			Inspector.Add(MethodBase.GetCurrentMethod().DeclaringType.FullName + "." + MethodBase.GetCurrentMethod().Name);
			var result = da.InsPartUpdateRireki(kinouId, userName, userId, tanaNo, hinban, hinmei, bumonCd, kaihatuPerson, updateProperty, atoData, series);
			da.Close();
			return result;
		}
	}
